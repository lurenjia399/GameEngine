...
namespace primitives
{
	bool is_inside_triangle(ftriangle in_triangle, int x, int y)
	{
		...
		if (fvector_2d::cross_product(
			...
			fvector_2d((float)x, (float)y) - in_triangle.point_1) < 0.f)
		{
		...
		if (fvector_2d::cross_product(
			in_triangle.point_3 - in_triangle.point_2,
			fvector_2d((float)x, (float)y) - in_triangle.point_2) < 0.f)
		{
		...
		if (fvector_2d::cross_product(
			...
			fvector_2d((float)x, (float)y) - in_triangle.point_3) < 0.f)
		{
			return false;//右侧 不在内部
		}
