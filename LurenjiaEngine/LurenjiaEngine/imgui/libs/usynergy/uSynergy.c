...
static void sProcessMessage(uSynergyContext *context, const uint8_t *message)
{
	...
	if (memcmp(message+4, "Synergy", 7)==0)
	{
		...
		else
		{
			...
			char buffer[256+1];
			sprintf_s(buffer, (size_t)(256 + 1), "Connected as client \"%s\"", context->m_clientName);
...
//		uint16_t count = sNetToNative16(message+12);
	...
	else
	{
		...
		//		kMsgEBad			= "EBAD"
		char buffer[64];
		sprintf_s(buffer, (size_t)64, "Unknown packet '%c%c%c%c'", message[4], message[5], message[6], message[7]);
		sTrace(context, buffer);
		return;
	}
...
static void sUpdateContext(uSynergyContext *context)
{
	...
	if (context->m_receiveFunc(context->m_cookie, context->m_receiveBuffer + context->m_receiveOfs, receive_size, &num_received) == USYNERGY_FALSE)
	{
		...
		char buffer[128];
		sprintf_s(buffer, (size_t)128, "Receive failed (%d bytes asked, %d bytes received), trying to reconnect in a second", receive_size, num_received);
		sTrace(context, buffer);
	...
	if (packlen > USYNERGY_RECEIVE_BUFFER_SIZE)
	{
		...
		char buffer[128];
		sprintf_s(buffer, (size_t)128, "Oversized packet: '%c%c%c%c' (length %d)", context->m_receiveBuffer[4], context->m_receiveBuffer[5], context->m_receiveBuffer[6], context->m_receiveBuffer[7], packlen);
		sTrace(context, buffer);
...
void uSynergySendClipboard(uSynergyContext *context, const char *text)
{
	...
	{
		char buffer[128];
		sprintf_s(buffer, (size_t)128, "Clipboard buffer too small, clipboard truncated at %d characters", max_length);
		sTrace(context, buffer);
