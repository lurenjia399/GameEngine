...
int CDirectXRenderingEngine::PostInit()
{
	...
	//	}
	//}
	if (ASphereMesh* SphereMesh_18 = World->CreateActor<ASphereMesh>("SphereMesh_18"))
	{
		SphereMesh_18->SetMeshComponent("SphereMesh_18Component", 2.f, 120, 120);
		SphereMesh_18->SetComponentPosition(XMFLOAT3(0.f, 13.f, 2.f));
		if (CMaterial* SphereMaterial_18 = new CMaterial())
		{
			SphereMaterial_18->ResetGuid("SphereMaterial_18");//给创建的材质设置Guid

			//SphereMaterial_18->SetMaterialTextureMapKey("MMOARPG");
			SphereMaterial_18->SetMaterialNormalMapKey("Wood2_Nor");
			SphereMaterial_18->SetRoughness(0.2f);
			//SphereMaterial_18->SetMaterialDisplayStatusType(EMaterialDisplayStatusType::TriangleDisplay);
			SphereMaterial_18->SetMaterialType(EMaterialType::BlinnPhone);
			SphereMesh_18->SetSubMaterials(0, SphereMaterial_18);
